...
bool Setup()
{
	...
	wcscpy(lf.lfFaceName, _T("Times New Roman")); // font style

	//
	// Create the font and select it with the device context.
	//
    hFont = CreateFontIndirect(&lf);    // 这里并非D3DXCreateFontIndirect,这里是WINapi
    hFontOld = (HFONT)SelectObject(hdc, hFont); 

	//
	// Create the text mesh based on the selected font in the HDC.
	//
    D3DXCreateText(                 // 创建三维文字模型
        Device,                     // 网格关联的设备 
        hdc,                        // 创建Mesh的文本设备上下文句柄 
        _T("Direct3D"),             // NULL结尾的文字
        0.001f,                     // 设置真实字形的外形偏移值,需要>=0 
        0.4f,                       // 字体在负Z轴方向上面的深度值 
        &Text,                      // 创建好的Mesh 
        0,                          // Mesh的邻接信息,一个DWORD数组
        0                           // 符号度量信息
        );
...
bool Display(float timeDelta)
{
	if( Device )
	{
        ...
        static d3d::CalcFPS g_FPS(Device,Width,Height);       
