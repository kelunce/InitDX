...
bool Setup()
{
	...
	if(FAILED(hr))
	{
		::MessageBox(0, _T("D3DXCompileShaderFromFile() - FAILED"), 0, 0);
		return false;
	}
	...
	if(FAILED(hr))
	{
		::MessageBox(0, _T("CreateVertexShader - FAILED"), 0, 0);
		return false;
	}
	...
	D3DXCreateTextureFromFile(Device, _T("crate.bmp"), &BaseTex);
	D3DXCreateTextureFromFile(Device, _T("spotlight.bmp"), &SpotLightTex);
	D3DXCreateTextureFromFile(Device, _T("text.bmp"), &StringTex);
