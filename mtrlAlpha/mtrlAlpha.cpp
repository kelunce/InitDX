...
*/
	...
	// use alpha in material's diffuse component for alpha 指定使用材质的漫反射的alpha分量作为alpha分量,本质上是指定alpha分量的来源
	Device->SetTextureStageState(0, D3DTSS_ALPHAARG1, D3DTA_DIFFUSE);   // 设置第0号纹理alpha值从顶点和材质中计算获得http://msdn.microsoft.com/zh-cn/library/ms886612.aspx
	Device->SetTextureStageState(0, D3DTSS_ALPHAOP, D3DTOP_SELECTARG1);// 该函数设置第0号纹理渲染状态来影响纹理渲染结果
