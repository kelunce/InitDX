...
bool Setup()
{
	...
	hr = D3DXCreateEffectFromFile(
		...
		_T("tooneffect.txt"), // effect filename
	...
	D3DXCreateTextureFromFile(Device, _T("toonshade.bmp"), &tex);
...
bool Display(float timeDelta)
{
	if( Device )
	{
		...
		{
			//ToonEffect->Pass(i); Dx9.0c以后已经不再使用这个函数了,使用BeginPass和EndPass
            ToonEffect->BeginPass(i);// 这个函数和EndPass配对,并且必须在Begin和End之间
			for(int j = 0; j < 4; j++)
			{
                ...
                //ToonEffect->CommitChanges(); 除非调用 Effect::Setx才需要执行本句
				Meshes[j]->DrawSubset(0);
			}
            ToonEffect->EndPass();// 这个函数和BeginPass配对,并且必须在Begin和End之间
		}
		ToonEffect->End();
